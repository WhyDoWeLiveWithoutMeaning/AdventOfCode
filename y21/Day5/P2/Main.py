...
def problem1(lines: list[Line]) -> int:
    ...
    for line in lines:
        if line.isStraight():
            ...
            if line.beginning[0] < line.end[0] and line.beginning[1] < line.end[1]:
                for x, i in zip(range(line.beginning[0], line.end[0]+1), range(line.end[0]+1)):
                    grid[line.beginning[1]+i][x] += 1
            elif line.beginning[0] < line.end[0] and line.beginning[1] > line.end[1]:
                for x, i in zip(range(line.beginning[0], line.end[0]+1), range(line.end[0]+1)):
                    grid[line.beginning[1]-i][x] += 1
            elif line.beginning[0] > line.end[0] and line.beginning[1] < line.end[1]:
                for x, i in zip(range(line.end[0], line.beginning[0]+1), range(line.beginning[0]+1)):
                    grid[line.end[1]-i][x] += 1
            elif line.beginning[0] > line.end[0] and line.beginning[1] > line.end[1]:
                for x, i in zip(range(line.end[0], line.beginning[0]+1), range(line.beginning[0]+1)):
                    grid[line.end[1]+i][x] += 1
